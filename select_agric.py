 ...
 ***************************************************************************/
"""
from PyQt5.QtCore import QSettings, QTranslator, QCoreApplication, QVariant
        ...
        if result:
           ...
           # acessa layer do CAR (limites de propriedades)
            ...
            if self.escolha == 1:
                ...
                # dir_layer = 'C:/PyQGIS/Dados/central_mineira_dissolv.shp'
                
                vlayer = QgsVectorLayer(dir_layer, "MG_Municipios_2022", "ogr")
                # vlayer = QgsVectorLayer(dir_layer, "CentralMineira", "ogr")
            ...
            area_select_agric_indiv = 'C:/PyQGIS/Area_SelectAgric_indiv_' + self.var + '.tif'

            processing.run(
            "grass:r.clump",
            {'input':area_select_agric,
            'title':'individualizacao de poligonos',
            '-d':False,
            'output':area_select_agric_indiv,
            'threshold':0,
            'GRASS_REGION_PARAMETER':None,
            'GRASS_REGION_CELLSIZE_PARAMETER':0,
            'GRASS_RASTER_FORMAT_OPT':'',
            ...
            # Relatorio (seleção de áreas para agricultura)
            report = 'C:/PyQGIS/report_' + self.var + '.txt'

            processing.run(
            "grass:r.report",
            {'map':[area_select_agric_indiv],
            'units':4,
            'null_value':'*',
            'page_length':0,
            'page_width':79,
            ...
            'GRASS_REGION_CELLSIZE_PARAMETER':0})
             ...
             # Resultado (seleção de áreas para agricultura)
            area_select_agric_indiv_vetor ='C:/PyQGIS/Area_SelectAgric_indiv_' + self.var + '.shp'

            processing.run("gdal:polygonize", 
            {'INPUT':area_select_agric_indiv,
            'BAND':1,
            'FIELD':'past',
            'EIGHT_CONNECTEDNESS':False,
            'EXTRA':'',
            'OUTPUT':area_select_agric_indiv_vetor})

             # carregando o resultado em tela

            vlayer_area_select_agric_indiv_vetor = QgsVectorLayer(area_select_agric_indiv_vetor, "area_select_agric_indiv_vetor", "ogr")
            QgsProject.instance().addMapLayer(vlayer_area_select_agric_indiv_vetor)

            # abre tabela de atributos do area_select_agric_indiv_vetor,
            # aciona edição, e calcula área por ha:
            # $area/10000 (em geometria) no campo area_ha

            # acessa layer
            layer = vlayer_area_select_agric_indiv_vetor
            # layer = iface.activeLayer()
                
            indiceCampo = layer.fields().indexOf('area_ha')

            provedor = layer.dataProvider()
            provedor.addAttributes([QgsField('area_ha', QVariant.Int)])
            layer.updateFields()
            novoIndice = layer.fields().indexOf('area_ha')
            ...
            for atributo in atributos:
                idx = atributo.id() # indice de linha
                area = atributo.geometry().area()
                resultado = {novoIndice:(area/10000)}
                provedor.changeAttributeValues({idx:resultado})
            layer.commitChanges()

            # executa overlay vetor intersection entre pastagem e propriedades
            area_select_agric_propried ='C:/PyQGIS/Area_SelectAgric_proried_' + self.var + '.shp'
            
            processing.run(
            "native:intersection",
            {'INPUT':area_select_agric_indiv_vetor,
            'OVERLAY':vlayer_car,
            'INPUT_FIELDS':[],
            'OVERLAY_FIELDS':['cod_imovel','mod_fiscal','num_area','des_condic','municipio'],
            'OVERLAY_FIELDS_PREFIX':'',
            'OUTPUT':area_select_agric_propried,
            'GRID_SIZE':None})

             # carregando o resultado em tela
            vlayer_area_select_agric_propried = QgsVectorLayer(area_select_agric_propried, "area_select_agric_propried", "ogr")
            QgsProject.instance().addMapLayer(vlayer_area_select_agric_propried)
